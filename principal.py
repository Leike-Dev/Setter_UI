...
def painel_boas_vindas_f() -> None:
    ...
    apresentar = Text('Seu assistente inteligente de renomeação de arquivos no terminal!', justify='center')

    # Painel de boas-vidas do programa
    panel_titulo = Panel(title='Bem-vindo ao Setter!', title_align='center', subtitle='𝛂', style=caixa_intro,
...
def nome_arquivo_formatado_f(nome_formatado: str) -> None:
    ...
    formatado = Text(f'{' ' * 1}{nome_formatado}', style=painel_arquivo_final)

    panel_final = Panel(title='Nome Gerado', title_align='left', style=painel_arquivo_final, box=box.ASCII2,
...
def tabela_escolha_f() -> int:
    opcoes = {1: 'Documento', 2: 'Imagem', 3: 'Vídeo', 4: 'Software', 5: 'Furry'}

    dados = opcoes

    titulo_tab = 'Tipo de Arquivo'
    coluna_1 = 'Opções'
    coluna_2 = 'Tipo'
